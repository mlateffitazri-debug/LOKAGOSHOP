...
function renderSellerCard(seller: Seller, index: number, lang: Lang, savedShopIds: Set<string>) {
...
function renderShopList(lang: Lang, sellers: Seller[], isLoading: boolean, error: string | null, savedShopIds: Set<string>) {
  if (isLoading) {
  ...
  return sellers.map((seller, index) => renderSellerCard(seller, index, lang, savedShopIds)).join('')
}
...
function renderHomeMarkup(lang: Lang, sellers: Seller[], isLoading: boolean, error: string | null, profile: HomeProfile | null, savedShopIds: Set<string>) {
  ...
  return replaceShopList(markup, renderShopList(lang, sellers, isLoading, error, savedShopIds))
}

function renderPageInner(markup: string) {
  const pagePrefix = '<div class="page">\n'
  const pageSuffix = '\n</div>'

  if (markup.startsWith(pagePrefix) && markup.endsWith(pageSuffix)) {
    return markup.slice(pagePrefix.length, -pageSuffix.length)
  }
...
export default function HomePage() {
  ...
  const markup = useMemo(
    () => renderHomeMarkup(lang, sellers, isLoading, error, profile, savedShopIds),
    [error, isLoading, lang, profile, sellers, savedShopIds],
  )
  const pageInnerMarkup = useMemo(() => renderPageInner(markup), [markup])
  ...
  async function handleClick(event: MouseEvent<HTMLDivElement>) {
    ...
    if (target.closest('.share-btn')) {
      ...
      if (navigator.share) {
        try {
          await navigator.share({ title: shopName, text: 'Tengok kedai ini di LokaGo!', url })
        } catch {
