...
import type { Buyer, Seller } from '@/types/database'
...
function AvatarCircle({ profile, size = 60 }: { profile: HomeProfile | null; size?: number }) {
  ...
  return (
    <div
      className="flex shrink-0 items-center justify-center overflow-hidden rounded-full border-2 border-white/45 bg-[#7B1533] font-extrabold text-white"
      style={{ width: dimension, height: dimension, fontSize: size >= 60 ? 24 : 15 }}
    >
      {profile?.avatarUrl ? (
...
function SidebarLink({ href, icon, label }: { href: string; icon: string; label: string }) {
  return (
    <a href={href} className="flex items-center gap-3 rounded-2xl bg-white px-4 py-3 text-sm text-[#111111] shadow-[0_2px_10px_rgba(0,0,0,0.05)]">
      <span className="flex h-9 w-9 items-center justify-center rounded-xl bg-[#F5F5F5] text-lg">{icon}</span>
...
export default function HomePage() {
  ...
  async function handleWhatsappSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    if (!profile?.buyerId) {
      setWhatsappStatus('Profil pembeli belum tersedia')
      return
    }
    ...
    const { error: updateError } = await supabase
      ...
      .eq('id', profile.buyerId)

    if (updateError) {
      console.error(updateError)
      setWhatsappStatus('Gagal simpan nombor WhatsApp')
      return
    }
  ...
  return (
    <>
      ...
      <div
        className={`fixed inset-0 z-50 bg-black/45 transition-opacity duration-300 ${isSidebarOpen ? 'opacity-100' : 'pointer-events-none opacity-0'}`}
        onClick={() => setIsSidebarOpen(false)}
        aria-hidden={!isSidebarOpen}
      >
        <aside
          className={`absolute right-0 top-0 flex h-full w-[86%] max-w-[360px] flex-col bg-[#F5F5F5] font-['Plus_Jakarta_Sans',sans-serif] shadow-[-18px_0_50px_rgba(0,0,0,0.28)] transition-transform duration-300 ease-out ${isSidebarOpen ? 'translate-x-0' : 'translate-x-full'}`}
          onClick={(event) => event.stopPropagation()}
        >
          <header className="bg-[#7B1533] px-5 pb-6 pt-6 text-white">
            <div className="flex items-center justify-between">
              <span className="text-sm font-bold text-white/65">Menu Profil</span>
              <button
                type="button"
                onClick={() => setIsSidebarOpen(false)}
                className="flex h-8 w-8 items-center justify-center rounded-full bg-white/15 text-xl leading-none"
            ...
            <div className="mt-5 flex items-center gap-4">
              ...
              <div className="min-w-0 flex-1">
                <p className="truncate text-lg font-extrabold">{profile?.name || 'LokalGo'}</p>
                <p className="mt-0.5 truncate text-xs text-white/60">{profile?.email || 'Belum log masuk'}</p>
                <span className="mt-2 inline-flex max-w-full rounded-full border border-white/15 bg-white/10 px-3 py-1 text-[11px] font-semibold text-white/75">
                  {profile?.kawasan || 'Kawasan belum ditetapkan'}
                </span>
              </div>
            </div>
          </header>
          ...
          <div className="flex-1 overflow-y-auto px-4 py-4">
            <div className="flex flex-col gap-3">
              <SidebarLink href="/profile/address" icon="📍" label="Alamat Penghantaran" />

              <form onSubmit={handleWhatsappSubmit} className="rounded-2xl bg-white p-4 shadow-[0_2px_10px_rgba(0,0,0,0.05)]">
                <label htmlFor="buyer-whatsapp" className="flex items-center gap-3 text-sm font-semibold text-[#111111]">
                  <span className="flex h-9 w-9 items-center justify-center rounded-xl bg-[#F5F5F5] text-lg">📱</span>
                  Tetapan WhatsApp
                </label>
                <div className="mt-3 flex gap-2">
                  <input
                    ...
                    className="min-w-0 flex-1 rounded-xl border border-[#E5E5EA] px-3 py-2 text-sm outline-none focus:border-[#7B1533]"
                  />
                  <button type="submit" className="rounded-xl bg-[#7B1533] px-3 py-2 text-xs font-bold text-white">
                    Simpan
                  </button>
                </div>
                {whatsappStatus ? <p className="mt-2 text-[11px] font-semibold text-[#7B1533]">{whatsappStatus}</p> : null}
              </form>

              <SidebarLink href="/saved" icon="❤️" label="Kedai Disimpan" />
              <SidebarLink href="/testimonials" icon="💬" label="Testimoni Saya" />
              <SidebarLink href="/sokong" icon="🙏" label="Sokong Pembangun" />
              <SidebarLink href="/about" icon="ℹ️" label="Tentang LokaGo" />

              <button
                type="button"
                onClick={toggle}
                className="flex items-center gap-3 rounded-2xl bg-white px-4 py-3 text-left text-sm text-[#111111] shadow-[0_2px_10px_rgba(0,0,0,0.05)]"
              >
                <span className="flex h-9 w-9 items-center justify-center rounded-xl bg-[#F5F5F5] text-lg">🌐</span>
                <span className="flex-1 font-semibold">Tukar Bahasa</span>
                <span className="rounded-full bg-[#ADD036] px-2.5 py-1 text-[10px] font-extrabold text-[#3D4D0E]">
                  {lang === 'ms' ? 'English' : 'BM'}
                </span>
              </button>
            </div>
          ...
          <div className="border-t border-[#E5E5EA] bg-white p-4">
            <button
              type="button"
              onClick={handleSignOut}
              className="w-full rounded-2xl border-2 border-[#7B1533] px-4 py-3 text-sm font-extrabold text-[#7B1533]"
            >
              Log Keluar
            </button>
          </div>
