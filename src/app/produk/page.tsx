...
export default function Page() {
  ...
  useEffect(() => {
    ...
    async function loadProduct() {
      ...
      if (breadcrumbShop) breadcrumbShop.textContent = currentSeller?.shop_name || ''
      const breadcrumbProduk = document.getElementById('breadcrumbProduk')
      if (breadcrumbProduk) breadcrumbProduk.textContent = displayName
      ...
      document.getElementById('preorderExplainBox')?.remove()
      if (currentProduct.is_preorder) {
        const minQtyHtml = currentProduct.min_qty_preorder && currentProduct.min_qty_preorder > 1
          ? `<div style="font-size:12px;font-weight:600;color:#856404;margin-top:6px;">Minimum tempahan: ${currentProduct.min_qty_preorder} unit</div>`
          : ''
        const box = document.createElement('div')
        box.id = 'preorderExplainBox'
        box.style.cssText = 'background:#FFF8E1;border:1px solid #FFE082;border-radius:10px;padding:11px 13px;margin-top:12px;'
        box.innerHTML = `<div style="font-size:12px;color:#856404;line-height:1.6;">Produk ini adalah Pre-Order. Sila pilih tarikh pickup dan confirm semula dengan seller melalui WhatsApp.</div>${minQtyHtml}`
        document.querySelector('.produk-desc-full')?.insertAdjacentElement('afterend', box)
      }

      setText('#addBtnTxt', currentProduct.is_preorder ? 'Tambah Pre-Order ke Troli' : 'Tambah ke Troli')
      ...
      // ── SALE MODE GATING ────────────────────────────────────────
      const saleMode = currentProduct.is_preorder ? 'PREORDER_ONLY' : 'DIRECT'
      ...
      runtime.addToOrder = async () => {
        ...
        if (btn) {
          ...
          window.setTimeout(() => {
            btn.classList.remove('added')
            btn.textContent = isPreorder ? 'Tambah Pre-Order ke Troli' : 'Tambah ke Troli'
          }, 1200)
        }
      }
