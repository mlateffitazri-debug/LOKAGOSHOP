...
export default function Page() {
  useEffect(() => {
    ...
    async function loadSeller() {
      ...
      if (inputs[2]) inputs[2].value = currentSeller.taman_name
      if (inputs[3]) inputs[3].value = currentSeller.postcode
    ...
    runtime.saveChanges = async () => {
      ...
      const tamanName = inputs[2]?.value.trim()
      const postcode = inputs[3]?.value.trim()
    ...
    async function onImageChange() {
      ...
      if (!file) return
      profileImageUrl = await readImageAsDataUrl(file)
      renderPhoto(profileImageUrl)
    }
